...
static const char* g_pJNIREG_CLASS = "com/acquiro/wpos/jni/PrinterInterface";
//	+add by pengli
...
int native_printer_open(JNIEnv* env, jclass obj) {
	...
	if (g_pPrinterInstance == NULL) {
		...
		if(NULL == (g_pPrinterInstance->read = (printer_read) dlsym(pHandle, methodName = "printer_read")))
		{
			hal_sys_error("can't find %s", methodName);
		}
...
int native_printer_read(JNIEnv* env, jclass obj, jbyteArray arryData, jint nDataLength, jint nTimeout) {
	hal_sys_info("+ native_printer_read()");
	int nResult = ERR_NORMAL;
	if (g_pPrinterInstance == NULL)
		return ERR_NOT_OPENED;
	if(g_pPrinterInstance->read == NULL)
		return ERR_NO_IMPLEMENT;

	jbyte* pData = env->GetByteArrayElements(arryData, NULL);
	nResult = g_pPrinterInstance->read((unsigned char*) pData, nDataLength, nTimeout);
	env->ReleaseByteArrayElements(arryData, pData, 0);
	hal_sys_info("- native_printer_read(),result = %d", nResult);
	return nResult;
}
...
//	return nResult;
//}
jboolean native_printer_is_opened(JNIEnv* env, jclass obj){
	jboolean isOpened = JNI_FALSE;
	hal_sys_info("native_printer_is_opened() is called\n" );
	if(g_pPrinterInstance != NULL){
		isOpened = JNI_TRUE;
	}
	hal_sys_info("native_printer_is_opened() end result = %d" , isOpened);
	return isOpened;
}

int native_printer_close(JNIEnv* env, jclass obj) {
	hal_sys_info("+ native_printer_close()");
	int nResult = ERR_NOT_OPENED;
	if(native_printer_is_opened(env, obj) == JNI_TRUE){
...
static JNINativeMethod g_Methods[] = {
		...
		{ "read", 				"([BII)I",			(void*) native_printer_read },
		...
		{"isOpened", 			"()Z",				(void*) native_printer_is_opened }
};
