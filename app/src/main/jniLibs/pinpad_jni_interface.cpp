...
const char* g_pJNIREG_CLASS = "com/acquiro/wpos/jni/PinPadInterface";
//	+add by pengli
static const char* g_pJNIREG_CLASS_INTERNAL =  "com/acquiro/wpos/jni/PinPadInterface";
//	-add by pengli
...
int native_pinpad_open(JNIEnv* env, jclass obj) {
	...
	if (g_pPinpadInstance == NULL) {
		...
		if(NULL == (g_pPinpadInstance->get_serial_number = (pinpad_get_serial_number) dlsym(pHandle, methodName = "pinpad_get_serial_number"))
				...
				| NULL == (g_pPinpadInstance->set_flag_allow_bypass_pin = (pinpad_set_flag_allow_bypass_pin) dlsym(pHandle, methodName = "pinpad_set_flag_allow_bypass_pin"))
				| NULL == (g_pPinpadInstance->encrypt_string_with_mode = (pinpad_encrypt_string_with_mode) dlsym(pHandle, methodName = "pinpad_encrypt_string_with_mode"))
				)
		{
			hal_sys_error("can't find %s", methodName);
		}

		hal_sys_info("find pinpad_get_hwserialno : %p ", (g_pPinpadInstance->get_hwserialno = (pinpad_get_hwserialno) dlsym(pHandle, methodName = "pinpad_get_hwserialno")));
		hal_sys_info("find pinpad_get_mac_for_snk : %p ", (g_pPinpadInstance->get_mac_for_snk = (pinpad_get_mac_for_snk) dlsym(pHandle, methodName = "pinpad_get_mac_for_snk")));
...
int native_pinpad_encrypt_string_with_mode(JNIEnv* env, jclass obj, jbyteArray pPlainText, jbyteArray pCipherTextBuffer,
	...
	nResult = g_pPinpadInstance->encrypt_string_with_mode((unsigned char*) pArrayPlainText, nPlainText,
											(unsigned char*) pArrayCipherTextBuffer, nCipherTextBuffer,
											(unsigned int) nMode, (unsigned char*) pArrayIV, (unsigned int) nIVLen);
